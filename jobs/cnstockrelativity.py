...
from fundc.models.stock import StockBaseInfo, CnStockDailyPrice, CnStockGroup
...
def run(output_path):
    ...
    base_infos = StockBaseInfo.objects.filter(Region=constant.Region.CN)
    base_infos = dict([(base_info.Code, base_info.Name) for base_info in base_infos])
    prices = CnStockDailyPrice.objects.filter(Date__gte=startdate, Date__lte=enddate).order_by('Code', 'Date')
...
def output(output_path, base_infos, codes, datas, clusterAssment):
    CnStockGroup.objects.all().delete()
    with open(output_path, 'wb') as f:
        ...
        for markIndex, codes in groups.iteritems():
            for code in codes:
                ...
                CnStockGroup(Code=code.encode('utf-8'), StockGroup=markIndex).save()
            f.write('\n')
